...
setInterval(async () => {
  const updatedRoute = await client.send(new CalculateRoutesCommand({
  ...
  updateMapRoute(polyline.decodeToLineStringFeature(
...
async function syncRoutes() {
  ...
  const results = await Promise.all(
    pendingRoutes.map(r => calculateRoute(r.origin, r.destination, {
...
class RouteService {
  ...
  async calculateRoute(origin, destination, forDisplay = true) {
    ...
    // Decode if necessary
    const geometry = format === "FlexiblePolyline"
      ? polyline.decodeToLineStringFeature(response.Routes[0].Legs[0].Geometry.LineString)
...
const { route, geometry } = await routeService.calculateRoute(origin, destination);
  ...
  Routes: [
    {
      Summary: {
        ...
        RouteBBox: [              // Bounding box [minLng, minLat, maxLng, maxLat]
      ...
      Legs: [                     // Array of route segments
        {
          Geometry: {
            LineString: [         // Array of [lng, lat] coordinates
              [-97.7431, 30.2672],
              [-97.7445, 30.2678],
              // ... more coordinates
            ]
          },
          Summary: { /* leg-specific summary */ },
          TravelSteps: [ /* navigation instructions */ ]
        }
      ]
    }
  ]
  ...
  map.on('load', () => {
    ...
    waypoints.forEach((wp, i) => {
      new maplibregl.Marker({ color: '#f59e0b' })
        .setLngLat(wp)
        .setPopup(new maplibregl.Popup().setHTML(`<strong>Stop ${i + 1}</strong>`))
        .addTo(map);
    });
...
console.log(`Toll-free route: ${route.Summary.Distance}m, ${route.Summary.Duration}s`);
```
...
async function getAlternativeRoutes() {
  ...
  // Route 1: Fastest
  const fastest = await client.send(new CalculateRoutesCommand({
    ...baseParams,
    OptimizeRoutingFor: "FastestRoute"
  }));

  // Route 2: Avoid tolls
  const noTolls = await client.send(new CalculateRoutesCommand({
    ...baseParams,
    OptimizeRoutingFor: "FastestRoute",
    Avoid: { Tolls: true }
  }));
  ...
  const shortest = await client.send(new CalculateRoutesCommand({
    ...baseParams,
